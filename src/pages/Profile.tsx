...
import { 
    ...
    Camera, Calendar, MapPin, Upload, FileText, Check, Building
} from 'lucide-react';
...
const ProfilePage: React.FC = () => {
    ...
    // DYNAMIC USER LOGIC: 
    // Fetches the username stored during login. Defaults to 'user1' (Jane Smith) for testing.
    ...
    return (
        <div style={{ height: '100vh', width: '100vw', backgroundColor: 'white', display: 'flex', fontFamily: "'Inter', sans-serif", overflow: 'hidden' }}>
            ...
            <div style={{ width: '320px', backgroundColor: '#0F172A', color: 'white', display: 'flex', flexDirection: 'column', padding: '40px 24px', borderRight: '1px solid #1E293B' }}>
                ...
                {/* USER PROFILE INFO - Using Icon instead of Image */}
                ...
                <nav style={{ flex: 1 }}>
                    ...
                    {sections.map((section) => (
                        <div 
                            ...
                            style={{ 
                                display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderRadius: '12px', marginBottom: '8px', cursor: 'pointer',
                                backgroundColor: currentStep === section.id ? '#1E293B' : 'transparent',
                                color: currentStep === section.id ? '#38BDF8' : '#94A3B8',
                                transition: 'all 0.2s'
                            }}
                        >
                            {section.icon}
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                <header style={{ padding: '24px 48px', backgroundColor: 'white', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 10 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                        <div style={{ textAlign: 'right' }}>
                            ...
                            <div style={{ fontSize: '11px', color: '#38BDF8', fontWeight: 600 }}>{currentStep * 25}% Completed</div>
                        </div>
                        <div style={{ width: '160px', height: '6px', backgroundColor: '#F1F5F9', borderRadius: '10px' }}>
                            <div style={{ width: `${currentStep * 25}%`, height: '100%', backgroundColor: '#38BDF8', borderRadius: '10px', transition: 'width 0.4s ease' }} />
                        </div>
                    </div>
                </header>

                <main style={{ flex: 1, overflowY: 'auto', padding: '30px 80px' }}>
                    <div style={{ width: '100%', maxWidth: '1100px' }}>
                        {currentStep === 1 && <IdentificationSection user={userData} />}
                        {currentStep === 2 && <ContactSection user={userData} />}
                        {currentStep === 3 && <EmploymentSection />}
                        {currentStep === 4 && <FinancialsSection />}
...
const IdentificationSection = ({ user }: { user: any }) => (
    <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
        <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#0F172A', marginBottom: '8px' }}>Section 1: Applicant Identification</h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '24px 40px' }}>
            ...
            <div style={{ marginBottom: '24px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 18px', backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '12px' }}>
                    <Calendar size={16} color="#94A3B8" />
                    <input 
                        type="date" 
                        defaultValue="1992-05-15"
                        style={{ border: 'none', backgroundColor: 'transparent', outline: 'none', width: '100%', fontSize: '15px', color: '#1E293B', cursor: 'text' }} 
                    />
                </div>
            </div>
...
const ContactSection = ({ user }: { user: any }) => {
    ...
    return (
        <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
            <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#0F172A', marginBottom: '8px' }}>Section 2: Contact Information</h2>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px 40px' }}>
                ...
                <div style={{ gridColumn: 'span 2', display: 'flex', alignItems: 'center', gap: '10px', marginTop: '-10px', marginBottom: '10px' }}>
                    <div 
                        onClick={() => setIsSameAddress(!isSameAddress)}
                        style={{ 
                            width: '20px', height: '20px', borderRadius: '6px', border: '2px solid #38BDF8', 
                            backgroundColor: isSameAddress ? '#38BDF8' : 'transparent', cursor: 'pointer',
                            display: 'flex', alignItems: 'center', justifyContent: 'center'
                        }}
                    >
                        {isSameAddress && <Check size={14} color="#0F172A" strokeWidth={3} />}
                    </div>
                    <span style={{ fontSize: '14px', color: '#475569', fontWeight: 500, cursor: 'pointer' }} onClick={() => setIsSameAddress(!isSameAddress)}>
...
const EmploymentSection = () => (
    <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
        <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#0F172A', marginBottom: '8px' }}>Section 3: Employment Details</h2>
...
const FinancialsSection = () => (
    <div style={{ animation: 'fadeIn 0.3s ease-out' }}>
        <h2 style={{ fontSize: '28px', fontWeight: 800, color: '#0F172A', marginBottom: '8px' }}>Section 4: Income and Financials</h2>
...
const selectStyle = { 
    width: '100%', padding: '14px 18px', backgroundColor: '#F8FAFC', 
    ...
    <div style={{ marginBottom: '24px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 18px', backgroundColor: '#F8FAFC', border: '1px solid #E2E8F0', borderRadius: '12px' }}>
            ...
            <input 
                ...
                style={{ border: 'none', backgroundColor: 'transparent', outline: 'none', width: '100%', fontSize: '15px', color: '#1E293B' }} 
            />
        </div>
    </div>
);
...
const primaryBtnStyle = {
    padding: '14px 40px', borderRadius: '14px', border: 'none', backgroundColor: '#0F172A', color: '#38BDF8', 
    fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '15px'
};

const secondaryBtnStyle = (disabled: boolean) => ({
    padding: '14px 30px', borderRadius: '14px', border: '1px solid #E2E8F0', backgroundColor: 'white', 
    color: disabled ? '#CBD5E1' : '#475569', fontWeight: 600, cursor: disabled ? 'not-allowed' : 'pointer',
